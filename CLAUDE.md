...
This is **cfl** - a C library template/scaffold project designed to be cloned and migrated to create new C libraries. The workflow is:
...
./test_cfl       # Run test executable directly
```
...
- **include/cfl/** - Public API headers (what users #include)
  - `cfl.h` - Main API header
  - `cfl_types.h` - Common types/definitions

- **src/** - Implementation files (not exposed to users)
  - `cfl.c` - Core implementation
  - `cfl_int.c` - Internal utilities (for helpers not in public API)
...
- **cmake/** - CMake modules
  - `cflConfig.cmake.in` - Package config template (enables find_package())
...
- CMake package config generation (enables `find_package(cfl)`)
...
- Headers → `/usr/local/include/cfl/`
- CMake config → `/usr/local/lib/cmake/cfl/`
...
**Test properties:**
- Test name: `cfl_unit_tests` (CTest)
...
2. Update `test/CMakeLists.txt`:
   - Add to `target_sources(test_cfl PRIVATE test_mymodule.c)`
